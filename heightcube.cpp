...
#define VERTEX_SHADER "heightcube-vertex.sdr"
#define FRAGMENT_SHADER "heightspec-fragment.sdr"

#define CORE_PROFILE true

class HeightSpecular : public SB6_BasicApp {
public:
    HeightSpecular(const char *imageFile);
...
private:
    GLint _projectionLoc, _xformLoc, _vpsLoc, _lightLoc, _heightLoc
        , _flattenLoc, _texLoc, _specLoc;
    bool _flatten;
    GLuint _vertexArray;
    Matrix4 _projection, _xform;
    Vector3H _light;
    TextureRef _heightMap, _texture, _specular;
...
HeightSpecular::HeightSpecular(const char *imageFile) {
    display = SDLDisplay::resizableDisplay("Height Mapped Cube", 400, 400, CORE_PROFILE);
    _heightMap = Texture::loadImage(imageFile, GL_TEXTURE1);
    _texture = Texture::loadImage("textures/crate.png", GL_TEXTURE2);
    _specular = Texture::loadImage("textures/crate_specular.png", GL_TEXTURE3);
...
void HeightSpecular::resizeWindow(int newX, int newY) {
...
void HeightSpecular::mouseMovementEvent(Uint8 buttons, int x, int y, int offsetX, int offsetY) {
    GLfloat ndcX, ndcY;
...
void HeightSpecular::keyEvent(SDL_Keysym &key, bool press) {
    if (!press) {
        return;
    }
...
void HeightSpecular::loadUniforms() {
    ...
    glUniform1i(_texLoc, 2);

    _specLoc = program.uniformLocation("specSampler");
    glUniform1i(_specLoc, 3);
...
bool HeightSpecular::buildShaderProgram(const char *mainVert, const char *mainFrag) {
    bool success = true;
    GLuint glerror;
...
bool HeightSpecular::appMain() {
    ...
    _xform = Matrix4::translate(0.0f, 0.0f, _zPlane) 
    ...
    glDrawArraysInstanced(_drawMode, 0, _totalVerts, 6);
...
int main( int argc, char* args[] ) {
    ...
    if (argc == 1) {
        map = std::string("textures/crate_height.png");
    } else if (argc == 2) {
    ...
    HeightSpecular thisApp = HeightSpecular(map.c_str());
